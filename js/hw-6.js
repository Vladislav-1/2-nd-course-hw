...
console.log('');
const products = ['яблоко', 'груша', 'слива', 'персик', 'абрикос', 'ЯблокО', 'Яблоко наливное', 'Яблоко Белый Налив'];
const search = 'Яблоко';
...
const returnArrayRandomNumber = (userNumber) => {
	let arr = [];
	...
	console.log(`Передано число ${userNumber}.`);
	console.log(``);
	console.log(`Длина массива равна: ${arr.length}.`);
	return arr;
}

console.log(`Массив случайных чисел: [${returnArrayRandomNumber(55)}].`);
...
Через 73 дня после текущей даты наступит: ${new Date(futureDate.setDate(currentDate.getDate() + 73))}`);
